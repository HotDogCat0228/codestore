const DEFAULT_URL = (process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001').replace(/\/$/, '');
...
export function setBaseUrl(url) {
  ...
  if (cleaned) {
    localStorage.setItem('codestore_api_url', cleaned);
  } else {
    localStorage.removeItem('codestore_api_url');
  }
}
...
export const api = {
  ...
  deployDirs: {
    list: () => request('GET', 'deploy/dirs'),
    save: (dirs) => request('PUT', 'deploy/dirs', { body: { dirs } }),
  },
  ...
  files: {
    ...
    // Raw binary upload — bypasses multipart to avoid browser/extension interference
    uploadRaw: async (projectId, file, filePath) => {
      const url = `${getBaseUrl()}/api/projects/${projectId}/upload-raw`;
      const res = await fetch(url, {
        method: 'PUT',
        headers: {
          'x-api-key': API_KEY,
    ...
    rawUrl: (projectId, filePath) => {
      ...
      const url = `${baseUrl}/api/projects/${projectId}/raw`;
